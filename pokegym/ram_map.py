...
def explore(env):
    ...
    r, c, map_n = position(env.game) # this is [y, x, z]
    env.seen_coords.add((r, c, map_n))
    if int(read_bit(env.game, 0xD81B, 7)) == 0: # pre hideout
        if map_n in poketower:
            exploration_reward = 0
        elif map_n in pokehideout:
            exploration_reward = (0.03 * len(env.seen_coords))
        else:
            exploration_reward = (0.02 * len(env.seen_coords))
    elif int(read_bit(env.game, 0xD7E0, 7)) == 0 and int(read_bit(env.game, 0xD81B, 7)) == 1: # hideout done poketower not done
        if map_n in poketower:
            exploration_reward = (0.03 * len(env.seen_coords))
        else:
            exploration_reward = (0.02 * len(env.seen_coords))
    elif int(read_bit(env.game, 0xD76C, 0)) == 0 and int(read_bit(env.game, 0xD7E0, 7)) == 1: # tower done no flute
        if map_n == 149:
            exploration_reward = (0.03 * len(env.seen_coords))
        elif map_n in poketower:
            exploration_reward = (0.01 * len(env.seen_coords))
        elif map_n in pokehideout:
            exploration_reward = (0.01 * len(env.seen_coords))
        else:
            exploration_reward = (0.02 * len(env.seen_coords))
    elif int(read_bit(env.game, 0xD838, 7)) == 0 and int(read_bit(env.game, 0xD76C, 0)) == 1: # flute gotten pre silphco
        if map_n in silphco:
            exploration_reward = (0.03 * len(env.seen_coords))
        elif map_n in poketower:
            exploration_reward = (0.01 * len(env.seen_coords))
            ...
            exploration_reward = (0.02 * len(env.seen_coords))
    elif int(read_bit(env.game, 0xD838, 7)) == 1 and int(read_bit(env.game, 0xD76C, 0)) == 1: # flute gotten post silphco
        if map_n in silphco:
            exploration_reward = (0.01 * len(env.seen_coords))
        elif map_n in poketower:
            exploration_reward = (0.01 * len(env.seen_coords))
...
def update_pokedex(env):
    for i in range(0xD30A - 0xD2F7):
        caught_mem = env.game.get_memory_value(i + 0xD2F7)
        seen_mem = env.game.get_memory_value(i + 0xD30A)
        for j in range(8):
            env.caught_pokemon[8*i + j] = 1 if caught_mem & (1 << j) else 0
            env.seen_pokemon[8*i + j] = 1 if seen_mem & (1 << j) else 0  

def update_moves_obtained(env):
    # Scan party
    for i in [0xD16B, 0xD197, 0xD1C3, 0xD1EF, 0xD21B, 0xD247]:
        if env.game.get_memory_value(i) != 0:
            for j in range(4):
                move_id = env.game.get_memory_value(i + j + 8)
                if move_id != 0:
                    if move_id != 0:
                        env.moves_obtained[move_id] = 1
                    if move_id == 15:
                        env.cut = 1
    # Scan current box (since the box doesn't auto increment in pokemon red)
    num_moves = 4
    box_struct_length = 25 * num_moves * 2
    for i in range(env.game.get_memory_value(0xda80)):
        offset = i*box_struct_length + 0xda96
        if env.game.get_memory_value(offset) != 0:
            for j in range(4):
                move_id = env.game.get_memory_value(offset + j + 8)
                if move_id != 0:
                    env.moves_obtained[move_id] = 1
